---
title: "Big Data for Social Issues: Project 1"
author: "Ben Warren"
date: "6/8/2022"
...
In this first empirical project, you will use the Opportunity Atlas mapping tool and the underlying data to describe equality of opportunity in your hometown and across the United States.

**Question 1:** Start by looking up the city where you grew up on the Opportunity Atlas. Zoom in to the Census tracts around your home.
...
In their 2018 paper, Chetty et al conceived their theory about the causal effect of moving between neighborhoods during childhood on adulthood outcomes. They found that there was a significant impact on adult's earnings based on when they moved into...
...
#Difference between home tract and overall
print(home_p25)
print(mean_kfr_pooled_25)
```
...
There is clearly more opportunity for upward mobility in my home tract than in the average tract in the U.S. The adult income of children with parents at the same 25th percentile mark in my home tract is over \$20,000/yr higher than that same metric for children from the average tract. This implies that the opportunity for economic mobility is quite high. I will qualify this by saying that the mean household income in my tract for 2000 was \$144,336, far higher than the average tract, with a 2010 poverty rate of 3.8%. It may be the case then, that this number represents a very small sample size, so it may be less telling than it seems.

**Question 4:** What is the standard deviation of upward mobility (population-weighted) in your home county? Is it larger or smaller than the standard deviation across tracts in your state? Across tracts in the country? What do you learn from these comparisons?
...
In the country, the standard deviation between tracts is 8169.155. This is smaller than in my county, but larger than in my state. It is closer to my state, though.
...
**Question 5:** Now let's turn to downward mobility: repeat questions (3) and (4) looking at children who start with parents at the 75th and 100th percentiles. How do the patterns differ?
...
#Difference between home tract and overall
print(home_p100)
print(mean_kfr_pooled_100)
```

Household income (\$) at age 31-37 for children with parents at the 75th percentile of the national income distribution (my home tract): \$78,572.56

Household income (\$) at age 31-37 for children with parents at the 75th percentile of the national income distribution (mean, all tracts): \$69,699.34
...
The standard deviation for adult incomes coming from the 75th percentile in my county is significantly lower than that at the state and nation level. I'm not sure what this means.
...
The linear regression model shows that there is a clear correlation between strong outcomes for low-income (25th percentile) children and high-income (75th percentile) children, with an R2 of 0.6756 (so nearly 70% of variance is explained by the independent variable) for a p-value of effectively 0.

**Question 7:** Next, examine whether the patterns you have looked at above are similar by race. If there is not enough racial heterogeneity in the area of interest (i.e., data is missing for most racial groups), then choose a different area to examine. (Not really working)
...
When we run regressions for each of the major racial groups here:
...
#reg model Black
plot(tracts_w_black$kfr_black_p25, tracts_w_black$kfr_black_p75, pch=16, main="Household Income (White), \n 75th percentile vs. 25th percentile", xlab = "25th Percentile Childhood Household Income", ylab = "75th Percentile Childhood Household Income")
lmodel3 <- lm(kfr_black_p75~kfr_black_p25, data=tracts_w_black)
abline(lmodel3, col="red", lwd=2)
```

```{r model summaries by race}
summary(lmodel1)
summary(lmodel2)
summary(lmodel3)
```
...
**Question 9:** Create a hypothesis about the variation in upward mobility in your county.
...
**Commute time:** Small correlation
